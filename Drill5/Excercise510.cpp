/*
10. Modify the calculator to operate on ints (only); give errors for overflow and underflow. Hint:
Use narrow_cast (§7.5)

//I don't get this one. The calculator operates on ints if Token value is int. In that case narrowing happens. Using narrow_cast throws exceptions for narrowing. As a result the program doesn't work.
furthermore i don't see how narrow_cast could be used to detect overflow except narrow_cast<int>(a+b) which is kind of pointless and will throw almost always.
*/
